...
| Ember-on-navy | `#EE5A28` | Added 2026-08-24 — ember-as-*text* on Navy only (eyebrow labels, `RecordSpine` claim-detail labels). 4.94:1, passes WCAG AA. Ember/ember-hover unchanged for backgrounds, icons, priority markers and text-on-white, where they already passed |
...
- **Ember eyebrow labels on Navy — fixed 2026-08-24.** New token `--color-ember-on-navy` (`#EE5A28`, 4.94:1 on Navy, passes AA) added specifically for ember-as-text on Navy backgrounds — `ember`/`ember-hover` are unchanged everywhere else. Applied to: Home hero, Building Record's "route does not end at submission" section, New Market hero, Security & Trust's hero, "Integrity checking" section and its three numbered markers (`app/security-trust/page.tsx` lines 62, 138, 158), `RecordNavigator`'s eyebrow, `SyntheticRecordDemo`, all three `RecordSpine` claim-detail `<dt>` labels, `RoleSolutionPage`'s `handover` variant. Two candidate fixes considered and not used: dropping to `on-navy-secondary`/white (safer margin but loses the ember accent on Navy entirely — see the comparison artifact linked from `DECISIONS.md` D-030), and enlarging/bolding to clear the 3:1 large-text threshold (impractical for the compact eyebrow style).
- **Still out of scope, noticed but not reviewed:** `RoleSolutionPage.tsx`'s "P1"/"P2" priority markers and "01"/"02" numbered list markers still use plain `text-ember` — a different UI pattern from the eyebrow label (priority/numbering, not a label), not folded into this fix.
